...
Leveldb 中采用了批量化写入的实现：请求到达之后会封装到一个 writer 结构体中，然后加到队列中。当线程不是 leader 的时候，就通过 condvar 等待唤醒，当线程是 leader 的时候（任务在最前面），就打包队列中的多个请求作为一个整体的请求写 wal 日志。写日志的时候会释放锁，让其它线程的写请求继续添加到任务队列中。写操作成功后，leader 会依次把这批任务的状态更新为完成并唤醒对应的 condvar，最后唤醒当前任务队列的 leader 结点让它开始处理下一批任务。
